...
public class RoutesApp extends RouteBuilder {
    ...
    public void configure() throws Exception {
        restConfiguration().component("jetty").host("localhost:8080" + configuration.getRestBaseUrl()).bindingMode(RestBindingMode.json);

        from("rest:put:items*").process(restInputProcessor).end();
